...
def crear():
    ...
    '''.format(miNombre.get(), miPass.get(), miApellido.get(), miDireccion.get(), cuadroTexto.get("1.0", END)))
...
def leer():
    ...
    ResultSetDeUsuario = miCursor.fetchall()

    for usuario in ResultSetDeUsuario:
        miId.set(usuario[0])
...
def actualizar():
    ...
    miCursor.execute('''
        UPDATE DATOSUSUARIOS SET NOMBRE_USUARIO='{}',
                     PASSWORD='{}',
                     APELLIDO='{}',
                     DIRECCION='{}',
                     COMENTARIOS='{}' WHERE ID='{}'
    '''.format(miNombre.get(), miPass.get(),miApellido.get(), miDireccion.get(), cuadroTexto.get("1.0", END), miId.get()))
